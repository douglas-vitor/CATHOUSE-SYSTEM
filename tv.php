...
function k_power(){
	...
	if ($tecla == 'KEY_POWER'){
		exec('irsend SEND_ONCE cathouse KEY_POWER');
	}
}
function k_vol_up(){
        $tecla = $_GET['btn'];
	$tecla = base64_decode($tecla);
        if ($tecla == 'KEY_VOL_UP'){
                exec('irsend SEND_ONCE cathouse KEY_VOL_UP');
        }
}
function k_vol_down(){
	$tecla = $_GET['btn'];
	...
	if ($tecla == 'KEY_VOL_DOWN'){
		exec('irsend SEND_ONCE cathouse KEY_VOL_DOWN');
	}
}
function k_um(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_UM'){
                exec('irsend SEND_ONCE cathouse KEY_UM');
        }
}
function k_dois(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_DOIS'){
                exec('irsend SEND_ONCE cathouse KEY_DOIS');
        }
}
function k_tres(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_TRES'){
                exec('irsend SEND_ONCE cathouse KEY_TRES');
        }
}
function k_quatro(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_QUATRO'){
                exec('irsend SEND_ONCE cathouse KEY_QUATRO');
        }
}
function k_cinco(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_CINCO'){
                exec('irsend SEND_ONCE cathouse KEY_CINCO');
        }
}
function k_seis(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_SEIS'){
                exec('irsend SEND_ONCE cathouse KEY_SEIS');
        }
}
function k_sete(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_SETE'){
                exec('irsend SEND_ONCE cathouse KEY_SETE');
        }
}
function k_oito(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_OITO'){
                exec('irsend SEND_ONCE cathouse KEY_OITO');
        }
}
function k_nove(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_NOVE'){
                exec('irsend SEND_ONCE cathouse KEY_NOVE');
        }
}
function k_zero(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_ZERO'){
                exec('irsend SEND_ONCE cathouse KEY_ZERO');
        }
}
function k_ch_up(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_CH_UP'){
                exec('irsend SEND_ONCE cathouse KEY_CH_UP');
        }
}
function k_ch_down(){
        $tecla = $_GET['btn'];
        ...
        if ($tecla == 'KEY_CH_DOWN'){
                exec('irsend SEND_ONCE cathouse KEY_CH_DOWN');
        }
}
function k_seta_cima(){
        ...
        if ($tecla == 'KEY_SETA_CIMA'){
                exec('irsend SEND_ONCE cathouse KEY_SETA_CIMA');
        }
}
function k_seta_baixo(){
        ...
        if ($tecla == 'KEY_SETA_BAIXO'){
                exec('irsend SEND_ONCE cathouse KEY_SETA_BAIXO');
        }
}
function k_seta_direita(){
        ...
        if ($tecla == 'KEY_SETA_DIREITA'){
                exec('irsend SEND_ONCE cathouse KEY_SETA_DIREITA');
        }
}
function k_seta_esquerda(){
        ...
        if ($tecla == 'KEY_SETA_ESQUERDA'){
                exec('irsend SEND_ONCE cathouse KEY_SETA_ESQUERDA');
        }
}
function k_enter(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_ENTER'){
                exec('irsend SEND_ONCE cathouse KEY_ENTER');
        }
}
function k_menu(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_MENU'){
                exec('irsend SEND_ONCE cathouse KEY_MENU');
        }
}
function k_exit(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_EXIT'){
                exec('irsend SEND_ONCE cathouse KEY_EXIT');
        }
}
function k_input(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_INPUT'){
                exec('irsend SEND_ONCE cathouse KEY_INPUT');
        }
}
function k_sound(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_SOUND'){
                exec('irsend SEND_ONCE cathouse KEY_SOUND');
        }
}
function k_pic_mode(){
        $tecla = $_GET['btn'];
        ...
        if ($tecla == 'KEY_PIC_MODE'){
                exec('irsend SEND_ONCE cathouse KEY_PIC_MODE');
        }
}
function k_pic_size(){
        $tecla = $_GET['btn'];
        ...
        if ($tecla == 'KEY_PIC_SIZE'){
                exec('irsend SEND_ONCE cathouse KEY_PIC_SIZE');
        }
}
function k_sleep(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_SLEEP'){
                exec('irsend SEND_ONCE cathouse KEY_SLEEP');
        }
}
function k_mute(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_MUTE'){
                exec('irsend SEND_ONCE cathouse KEY_MUTE');
        }
}
function k_info(){
        $tecla = $_GET['btn'];
        $tecla = base64_decode($tecla);
        if ($tecla == 'KEY_INFO'){
                exec('irsend SEND_ONCE cathouse KEY_INFO');
        }
}
